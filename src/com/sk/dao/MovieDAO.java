...
public class MovieDAO implements WorkDiv<MovieDTO> {
	...
	public int add(MovieDTO movie) {
		try (BufferedWriter bw = new BufferedWriter(new FileWriter(FILE_PATH, true))) {
			bw.newLine();
			bw.write(String.join(",", movie.getTitle(), String.valueOf(movie.getYear()), movie.getCountry(),
					movie.getRuntime(), movie.getGenre(), movie.getDirector()));
			return 1; // 성공
		} catch (IOException e) {
			e.printStackTrace();
			return 0; // 실패
	...
	public List<MovieDTO> getAll() {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(FILE_PATH))) {
			...
			while ((line = br.readLine()) != null) {
				...
				if (row.length < 6)
					continue;
				MovieDTO dto = new MovieDTO(row[0], Integer.parseInt(row[1].trim()), row[2], row[3], row[4], row[5]);
				list.add(dto);
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	private void writeAll(List<MovieDTO> data) {
		try (BufferedWriter bw = new BufferedWriter(new FileWriter(FILE_PATH))) {
			...
			for (MovieDTO m : data) {
				bw.newLine();
				bw.write(String.join(",", m.getTitle(), String.valueOf(m.getYear()), m.getCountry(), m.getRuntime(),
						m.getGenre(), m.getDirector()));
			}
		} catch (FileNotFoundException e) {
			System.out.println("❗ 파일이 열려 있어 쓸 수 없습니다. 엑셀/메모장 등에서 파일을 닫아주세요.");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
